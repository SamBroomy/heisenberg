...
pub fn get_raw_data() -> Result<(LazyFrame, LazyFrame, LazyFrame)> {
    ...
    let all_countries_df = all_countries::get_all_countries(&client)?;
    let country_info_df = country_info::get_country_info(&client)?;
    let feature_codes_df = feature_codes::get_feature_codes(&client)?;
