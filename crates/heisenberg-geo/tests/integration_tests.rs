...
use heisenberg_geo::{DataSource, LocationSearcher, SearchConfigBuilder};

// Counter for unique test IDs to enable parallel execution
static TEST_COUNTER: AtomicUsize = AtomicUsize::new(0);

fn setup_test_env() {
    let _ = heisenberg_geo::init_logging(tracing::Level::WARN);
}
...
fn create_test_searcher() -> LocationSearcher {
    ...
    let temp_dir = std::env::temp_dir()
        .join("heisenberg_integration_tests")
    ...
    let data = heisenberg_geo::LocationSearchData::new_embedded();
    let admin_data = data.admin_search_df();
    let places_data = data.place_search_df();

    // Create indexes with custom paths
    let admin_index = heisenberg_geo::FTSIndex::new(
        heisenberg_geo::AdminIndexDef,
        admin_data,
        &admin_index_path,
        false,
    )
    ...
    let places_index = heisenberg_geo::FTSIndex::new(
        heisenberg_geo::PlacesIndexDef,
        places_data,
        &places_index_path,
        false,
    )
    ...
    let indexes = heisenberg_geo::LocationSearchIndex {
        admin: admin_index,
        places: places_index,
    };
...
fn test_configuration_presets() {
    ...
    for (name, config) in configs {
        let results = searcher
            ...
            .unwrap_or_else(|_| panic!("{name} config should work"));

        println!("{} config: found {} results", name, results.len());
        assert!(
            results.len() <= config.limit,
            "Should respect limit for {name}"
        );
    }
}
...
fn test_error_handling() {
    ...
    for case in edge_cases {
        ...
        assert!(
            result.is_ok(),
            "Search should not error for edge case: {case:?}"
        );
        ...
        assert!(
            resolved.is_ok(),
            "Resolution should not error for edge case: {case:?}"
        );
    }
}
...
fn test_search_result_properties() {
    ...
    if let Some(result) = results.first() {
        ...
        match result {
            heisenberg_geo::SearchResult::Admin(_) => {
                println!(
                    "Found administrative entity: {}",
                    result.name().unwrap_or("Unknown")
                );
            }
            heisenberg_geo::SearchResult::Place(_) => {
    ...
    let handles: Vec<_> = (0..3)
        .map(|i| {
            ...
            thread::spawn(move || {
                ...
                assert!(results.is_ok(), "Concurrent search {i} should work");
                results.unwrap()
            })
        })
        .collect();
...
fn test_constructor_patterns() {
    ...
    let embedded_searcher = create_test_searcher();
