...
// Include the auto-generated metadata hash constant
include!(concat!(env!("OUT_DIR"), "/metadata_hash.rs"));

/// Get the user cache directory for indexes, using metadata hash for invalidation
/// Returns ~/.cache/heisenberg-geo/indexes/{metadata_hash}/ on Linux,
/// ~/Library/Caches/heisenberg-geo/indexes/{metadata_hash}/ on macOS
fn get_index_cache_dir() -> Option<PathBuf> {
    directories::ProjectDirs::from("", "", "heisenberg-geo")
        .map(|dirs| dirs.cache_dir().join("indexes").join(METADATA_HASH))
}
...
impl LocationSearchIndex {
    ...
    pub fn new(data: &LocationSearchData, overwrite: bool) -> Result<Self> {
        ...
        // Get the index path from cache (using metadata hash for auto-invalidation)
        let base_index_path = get_index_cache_dir().unwrap_or_else(|| {
            // Fallback to old location if cache dir unavailable
            warn!("Cache directory unavailable, using project directory for indexes");
            data.data_source().data_source_dir().join(Self::INDEX_DIR)
        });
    ...
    pub fn load_existing(
        ...
        if let (Some(admin), Some(places)) = (admin_index, places_index) {
            info!("Successfully loaded both existing indexes");
            Ok(Some(Self { admin, places }))
        } else {
...
impl<D: IndexDefinition> FTSIndex<D> {
    ...
    fn safely_recreate_dir(path: &Path) -> Result<()> {
        if path.exists() {
    ...
    fn search_inner(
        ...
        let top_docs =
            searcher.search(&*query, &TopDocs::with_limit(params.limit).order_by_score())?;
