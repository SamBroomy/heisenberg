...
fn backfill_administrative_context(
    ...
    let collected_frames = {
        ...
        let plans = flat_lazy_frames
            .into_iter()
            .map(|lf| lf.logical_plan)
            .collect();
        LazyFrame::collect_all_with_engine(plans, Engine::Auto, OptFlags::default())?
    };
...
pub fn resolve_search_candidate(
    ...
    let primary_candidates_df = match search_results.last() {
        Some(df) if df.height() != 0 => df.clone(),
        _ => {
