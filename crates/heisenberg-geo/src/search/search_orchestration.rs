...
///
/// ```rust
/// use heisenberg_geo::{LocationSearcher, SearchResult};
///
...
/// ```rust
/// use heisenberg_geo::SearchConfig;
///
...
/// ```rust
/// use heisenberg_geo::SearchConfigBuilder;
///
...
fn execute_admin_search_batches(
    ...
    batches
        ...
        .map(|batch| {
            ...
            match search_result {
                Ok(Some(result_df)) if result_df.height() != 0 => {
...
fn update_admin_filter_for_next_pass(
    ...
    if concatenated_results.height() == 0 {
    ...
    match potential_next_active_lf {
        Ok(active_admin_df) if active_admin_df.height() != 0 => {
            debug!(
