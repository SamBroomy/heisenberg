...
use polars::{
    ...
    lazy::dsl::mean_horizontal,
    prelude::{
        AnyValue, Column, DataType, Expr, Field, IntoLazy, LazyFrame, UnionArgs, as_struct, col,
        concat, lit, when,
    },
    series::Series,
};
...
pub(super) fn parent_factor(lf: LazyFrame) -> Result<LazyFrame> {
    ...
    let lf = if parent_score_exprs_for_mean.is_empty() {
        ...
        lf.with_column(
            mean_horizontal(parent_score_exprs_for_mean, true)?
                .fill_null(0.5_f32)
...
pub(super) fn filter_data_from_previous_results(
    ...
    // is empty was removed in `0.53` for some reason and this was the code that was there before.
    if unique_previous_paths_df.height() == 0 {
