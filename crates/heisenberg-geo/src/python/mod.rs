...
impl PyLocationSearcher {
    ...
    fn py_new(py: Python<'_>) -> PyResult<Self> {
        py.detach(|| match LocationSearcher::new_embedded() {
    ...
    fn search(&self, py: Python<'_>, input_terms: Vec<String>) -> PyResult<Vec<PySearchResult>> {
        py.detach(|| match self.inner.search(&input_terms) {
            Ok(results) => {
    ...
    fn resolve_location(
        ...
        let resolved_results_rust = py.detach(|| self.inner.resolve_location(&input_terms));
    ...
    fn resolve_location_with_config(
        ...
        let resolved_results_rust = py.detach(|| {
            self.inner
    ...
    fn resolve_location_batch(
        ...
        let resolved_results_rust =
            py.detach(|| self.inner.resolve_location_batch(&input_terms_batch));
...
impl PyLocationSearcherBuilder {
    ...
    fn build(&self, py: Python<'_>) -> PyResult<PyLocationSearcher> {
        py.detach(|| match self.inner.clone().build() {
...
#[pymodule]
#[pyo3(name = "_heisenberg_geo_rust_bindings")]
fn heisenberg_geo(m: &Bound<'_, PyModule>) -> PyResult<()> {
