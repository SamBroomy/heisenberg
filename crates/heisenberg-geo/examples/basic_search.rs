...
use heisenberg_geo::{LocationSearcher, SearchConfigBuilder};
...
fn print_search_results(results: &[heisenberg_geo::SearchResult], limit: usize) {
    for (i, result) in results.iter().take(limit).enumerate() {
        let result_type = match result {
            heisenberg_geo::SearchResult::Admin(_) => "Admin",
            heisenberg_geo::SearchResult::Place(_) => "Place",
        };
