...
async fn tweets(Extension(ctx): Extension<Arc<Mutex<Oauth2Ctx>>>) -> impl IntoResponse {
    ...
    std::fs::write(
        ".token.json",
        serde_json::to_string_pretty(&oauth_token).unwrap(),
    )
    .unwrap();
...
async fn main() {
    ...
    tracing_subscriber::registry()
        .with(EnvFilter::new(std::env::var("RUST_LOG").unwrap_or_else(
            |_| "oauth2_callback=debug,tower_http=debug".into(),
        )))
    ...
    tracing::debug!("serving at {}", listener.local_addr().unwrap());
